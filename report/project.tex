...
dupl -t 15 -html ./code/class01/student01/assignment02/ ./code/class01/student02/assignment02/ ./code/class01/student03/assignment02/ > assignment02.html &
			...
			\begin{lstlisting}[language=bash, breaklines=true]		
java -jar ./jplag/jplag.jar -r ./results/lab1 -l java17 -s lab1 ./students
			\end{lstlisting}
			\noindent The first three arguments say to run a java jar file in that location called jplag.jar. The next argument, $-r$ specifies where to save the results. $-l$ says which language to use, which in this case is Java 1.7. $-s$ says to check all the files in subdirectories with that label. So here any files in subdirectories labeled lab1 will be checked. The last argument is the base directory where the code resides.
